    ...
    for(i=0;i<task_set_object->num_tasks;i++)
    {
        ...
        for(j=0;j<num_jobs_of_task;j++)
        {
            ...
            for(k=0;k<num_resources;k++)
            {
                ...
                req_resource_info_object->list_head[k]->currently_holding=0;
...
*/
void scheduler_priority_inheritance(task_set *priority_inheritance_task_set, resource_list* resource_list_object, int scheduling_algo)
{
    ...
    while(decision_point_time<=priority_inheritance_task_set_hyperperiod)
    {
        ...
        if(current_executing_job!=NULL)
        {
            ...
            {   
                ...
                if(current_executing_job->resources_needed->list_head[i]->currently_holding==1)
                {
                    ...
                    if(current_executing_job->resources_needed->list_head[i]->remaining_time_needed==0)
                    {
                        ...
                        current_executing_job->resources_needed->list_head[i]->currently_holding=0;
        ...
        {
            if(((current_executing_job->execution_time-current_executing_job->remaining_execution_time)==current_executing_job->resources_needed->list_head[resource_needed_index]->time_acquired_at_since_execution) && current_executing_job->resources_needed->list_head[resource_needed_index]->currently_holding==0)
            {   
                ...
                if(req_resource_usage->instances_needed<=req_resource_usage->resource_name->remaining_instances)
                {
                    ...
                    req_resource_usage->currently_holding=1;
        ...
        for(int i=0;i<current_executing_job->resources_needed->number_of_resources;i++)
        {
            if(current_executing_job->resources_needed->list_head[i]->currently_holding==0)
            {
